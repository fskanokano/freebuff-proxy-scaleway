// -refresh-token mode (issue #66): re-authenticate one pooled token in .env
// via the headless GitHub login flow — the same /api/auth/cli/code +
// /api/auth/cli/status endpoints the dashboard wizard (#62) and the CLI
// use. Interactive: start → print login URL → poll → replace token #N in
// .env atomically. Non-interactive (-yes): with GITHUB_USER/
// GITHUB_PASSWORD/GITHUB_TOTP present, attempt the reference GitHub
// password+TOTP protocol login; otherwise print the login URL and exit 2
...
// runTokenRefresh drives the -refresh-token mode and exits. index is the
// 0-based AUTH_TOKENS position to replace.
func runTokenRefresh(configPath string, index int, autoYes bool) {
