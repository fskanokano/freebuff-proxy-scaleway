// Service management: -install-service / -uninstall-service / -service-status.
//
...
//
// Every builder is a pure function (unit/plist/task args + status parsers)
// so the wiring is unit-testable on any platform; the exec wrappers only run
// on the host platform when the flag is actually used.
package main
...
// runServiceInstall registers the current binary as a background service and
// starts it. Prints a human result and exits 0/1.
...
// runServiceUninstall stops and unregisters the service. Missing registration
...
// runServiceStatus reports whether the service is registered and active.
