...
func Load(configPath string) (Config, error) {
	...
	// Scaleway: PORT is injected by the platform (console Port param). When
	// LISTEN_ADDR was not explicitly set, bind to :$PORT (0.0.0.0:$PORT)
	// so the container passes the TCP healthcheck. Keeps local default
	// 127.0.0.1:3457 when PORT is unset.
	if _, hasListen := os.LookupEnv("LISTEN_ADDR"); !hasListen && raw.ListenAddr == "127.0.0.1:3457" {
	...
	// Scaleway aliases: FREEBUFF_TOKEN / FREEBUFF_TOKENS / TOKEN are
	// accepted as AUTH_TOKENS to keep the 3-var UX (proxy api key,
	// freebuff token, admin token) minimal.
	if v, ok := os.LookupEnv("AUTH_TOKENS"); ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := os.LookupEnv("FREEBUFF_TOKEN"); ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := os.LookupEnv("FREEBUFF_TOKENS"); ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := os.LookupEnv("TOKEN"); ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	}
	...
	// Scaleway alias: API_KEY / FREEBUFF_API_KEY / PROXY_API_KEY map to API_KEYS
	if v, ok := lookupFirstEnv("API_KEYS", "API_KEY", "FREEBUFF_API_KEY", "PROXY_API_KEY"); ok {
		raw.APIKeys = splitList(v)
	}
	overrideStringAlias(&raw.AdminToken, os.Getenv, "ADMIN_TOKEN", "ADMIN_PASSWORD")
...
func applyDotenv(raw *rawConfig, path string) error {
	...
	// Scaleway aliases: FREEBUFF_TOKEN / FREEBUFF_TOKENS / TOKEN in .env
	if v, ok := vals["AUTH_TOKENS"]; ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := vals["FREEBUFF_TOKEN"]; ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := vals["FREEBUFF_TOKENS"]; ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	} else if v, ok := vals["TOKEN"]; ok {
		raw.AuthTokens = splitList(v)
		raw.AuthTokensSet = true
	}
	overrideStringFrom(&raw.ListenAddr, get, "LISTEN_ADDR")
	// Scaleway .env PORT fallback not needed; real env PORT wins in Load.
	...
	// Scaleway alias: API_KEY variants for .env
	if v, ok := lookupFirstFromMap(vals, "API_KEYS", "API_KEY", "FREEBUFF_API_KEY", "PROXY_API_KEY"); ok {
		raw.APIKeys = splitList(v)
	}
	overrideStringAliasFromVals(vals, &raw.AdminToken, "ADMIN_TOKEN", "ADMIN_PASSWORD")
...
// Scaleway helpers: minimal alias support for 3-var UX + PORT fallback.
...
func lookupFirstFromMap(vals map[string]string, names ...string) (string, bool) {
	for _, n := range names {
		if v, ok := vals[n]; ok && strings.TrimSpace(v) != "" {
			return v, true
		}
	}
	return "", false
...
func overrideStringAliasFromVals(vals map[string]string, target *string, primary, alias string) {
	get := func(name string) string { return vals[name] }
	overrideStringAlias(target, get, primary, alias)
}
